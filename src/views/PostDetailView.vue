<template>
      ...
      <div class="flex flex-col gap-1 ml-2">
        <div class="p-1 flex gap-3">
          <div class="flex items-center gap-1" @click="() => toggleLikes()">
            <i
              class="pi"
              :class="[likedByMe ? 'pi-thumbs-up-fill' : 'pi-thumbs-up']"
  ...
  <Dialog
    class="mx-5"
    v-model:visible="removePostModalOpen"
    modal
    header="Do you really want to remove this post?"
  >
    <div class="flex gap-5">
      ...
      <button class="text-black text-lg" @click="removePostModalOpen = false">
        Cancel
      </button>
    </div>
  </Dialog>
...
const postCaption = computed(
  () => post.value?.caption || 'No content available',
)
const backendBaseurl = computed(() => import.meta.env.VITE_API_URL)
const profileImageSrc = computed(
  () => `${backendBaseurl.value}${post.value?.profile_picture_url}`,
)

const isMyPost = computed(
  () => authStore.userData.user_id === post.value?.user_id,
)

const removePostModalOpen = ref(false)

async function removePost() {
  const success = await userProfileStore.deletePost(
    post?.value.id,
    authStore.userData.user_id,
  )
  if (success) {
