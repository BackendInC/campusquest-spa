<template>
  <div class="min-h-screen bg-[#803049] p-4 relative">
    <div class="overflow-hidden absolute top-0 left-0 right-0 z-0 max-w-full">
      <img
        :src="honeycomb"
        class="object-cover -translate-y-40 translate-x-12 opacity-45 scale-150"
      />
    </div>
  </div>
  ...
  <div class="absolute top-4 left-4 z-10 mt-12 gap-3">
    <RouterLink
      to="/profile"
      class="pi pi-arrow-left text-white text-lg mr-8"
    ></RouterLink>
  ...
  <div
    class="bg-white w-full rounded-t-[2rem] px-4 pt-6 mt-36 mb-0 absolute top-0 left-0 z-10 h-full"
  >
    <div class="flex flex-row items-center mb-2">
      <img
        @click="openEditImage"
        :src="authStore.userData.profilePhoto"
        class="cursor-pointer w-20 h-20 rounded-lg mb-4"
      />
      <h2
        @click="openEditName"
        class="cursor-pointer text-gray-600 ml-3 text-2xl"
      >
        {{ authStore.userData.username }}
      </h2>
      <i
        @click="openEditName"
        class="cursor-pointer pi-pencil pi text-lg text-gray-600 pl-1"
      ></i>
    </div>
    ...
    <div class="flex flex-col ml-2 mt-2 gap-4">
      ...
      <div class="flex flex-col ml-2 mt-2 gap-4">
        ...
        <p @click="visible = true" class="text-red-700 text-lg">Log Out</p>
      </div>
    </div>
  </div>
  ...
  >
    <div class="flex flex-col gap-2">
      ...
      <button class="text-red-700 text-lg" @click="logOutAndRedirect">
        Log Out
      </button>
      <hr class="border-[#b2b1b1]" />
      <button class="text-black text-lg" @click="visible = false">
        Cancel
      </button>
    </div>
  </Dialog>
  ...
  <Dialog
    v-model:visible="imageInputMode"
    modal
  ...
  >
    <div class="flex items-center gap-4 mb-4">
      <InputText
        :modelValue="authStore.userData.name"
        v-model="nameBuffer"
        class="flex-auto flex-1 border rounded-md"
        required
      />
    </div>
    <div class="flex justify-end gap-2">
  ...
  <Dialog
    v-model:visible="emailInputMode"
    :draggable="false"
    modal
    header="Change Email"
    :style="{ width: '25rem' }"
  >
    <div class="flex items-center gap-4 mb-4">
      <InputText
        :modelValue="authStore.userData.email"
        v-model="emailBuffer"
        class="flex-auto flex-1 border rounded-md"
        required
      />
  ...
  <Dialog
    v-model:visible="passwordInputMode"
    :draggable="false"
    modal
  ...
  >
    <Form v-slot="$form" :initialValues :resolver @submit="onPasswordSubmit">
      ...
      <div class="flex items-center gap-4 mb-4">
        <label for="password_new" class="font-semibold w-24">
      ...
      <div class="flex items-center gap-4 mb-4">
        <label for="password_confirm" class="font-semibold w-24">
      ...
      <Message
        v-if="$form.password_new?.invalid"
        severity="error"
        size="small"
        variant="simple"
...
function updateName() {
  authStore.updateUserName(nameBuffer.value)
...
function updateEmail() {
  authStore.updateEmail(emailBuffer.value)
